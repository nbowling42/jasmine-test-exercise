...
function setupIntialValues() {
  const defaulValues = { amount: 35000, years: 6, rate: 5 };
  const loanAmount = document.querySelector('#loan-amount');
  loanAmount.value = defaulValues.amount;
  const loanYears = document.querySelector('#loan-years');
  loanYears.value = defaulValues.years;
  const loanRate = document.querySelector('#loan-rate');
  loanRate.value = defaulValues.rate;
  update();
}
...
function calculateMonthlyPayment(values) {
  ...
  let result = (values.amount * i) / (1 - (1 + i) ** -n);
